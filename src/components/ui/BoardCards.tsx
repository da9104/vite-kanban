    ...
    <div className="w-full mx-auto max-w-full rounded-lg bg-card uppercase">
      ...
      >
        {boards.map((board, index) => (
          <div
            key={board.id}
            className="flex flex-col p-8! gap-4 border border-gray-400 rounded-xl"
          >
            <div className="flex items-center gap-3">
              <span className="flex items-center justify-center w-8 h-8 rounded-md border border-gray-400 text-sm text-muted-foreground">
                {index + 1}
              </span>
